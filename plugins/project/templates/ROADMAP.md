# Roadmap: [name]

v[N], updated [YYYY-MM-DD]. Mode: [lite | full]. Source: `planning/roadmap.yaml`. Log: `planning/PROGRESS.md`.

Status words: **done** (green, frozen scope), **caveats** (yellow, works with named gaps), **focus** (red, where attention goes), **planned** (not started).

## Now
- [id] [name]: [gate or verify] ([status])

## Next
- [id] [name]: [gate or verify]

## Later
- [id] [name]

## Done
- [id] [name]: [evidence, source and date]

## Ideas (parked)
- [YYYY-MM-DD] [text] ([source])

## Invariants
- [what must never break]

<!-- full mode only -->
## Stages
| # | Stage | Status | Gate | Evidence / caveats | Refs |
|---|---|---|---|---|---|
| S1 | [name] | focus | [gate] | [evidence or caveats] | [] |
<!-- end full mode -->
