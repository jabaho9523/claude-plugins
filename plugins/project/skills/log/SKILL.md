...
1. **Which project.** Follow `${CLAUDE_PLUGIN_ROOT}/shared/resolve-project.md`. The text after the name is the row.
2. **One row.** `| YYYY-MM-DD | what, one line | refs | by |`
   - `what`: from the argument or the user's last message. Decisions are prefixed `Decision:`, discoveries `Finding:`.
...
3. **Append** to `planning/PROGRESS.md`. Reply with the row and nothing else.
